...
def main():
    ...
    A_ub = [
        ...
        hardness_coeffs,   # Hardness max constraint
        -tensile_coeffs,   # Tensile strength min constraint
        tensile_coeffs     # Tensile strength max constraint
    ]
    b_ub = [
        -target_data[target_data["Property"] == "Hardness"]["Min"].values[0],
        target_data[target_data["Property"] == "Hardness"]["Max"].values[0],
        -target_data[target_data["Property"] == "Tensile Strength"]["Min"].values[0],
        target_data[target_data["Property"] == "Tensile Strength"]["Max"].values[0]
    ]

    # Solve the optimization problem
    res = linprog(
        c=np.nan_to_num(composition_data["Cost"].values),
